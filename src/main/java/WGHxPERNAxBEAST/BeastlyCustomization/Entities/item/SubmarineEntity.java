package WGHxPERNAxBEAST.BeastlyCustomization.Entities.item;
...
public class SubmarineEntity extends Entity {
   private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(SubmarineEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(SubmarineEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(SubmarineEntity.class, DataSerializers.FLOAT);
   private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(SubmarineEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Boolean> field_199704_e = EntityDataManager.createKey(SubmarineEntity.class, DataSerializers.BOOLEAN);
   private static final DataParameter<Boolean> field_199705_f = EntityDataManager.createKey(SubmarineEntity.class, DataSerializers.BOOLEAN);
   private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(SubmarineEntity.class, DataSerializers.VARINT);
   ...
   private SubmarineEntity.Status status;
   private SubmarineEntity.Status previousStatus;
   ...
   public SubmarineEntity(EntityType<? extends SubmarineEntity> type, World worldIn) {
      super(type, worldIn);
      this.preventEntitySpawning = true;
   }

   public SubmarineEntity(World worldIn, double x, double y, double z) {
      this(EntitiesList.BASIC_SUB, worldIn);
   ...
   protected void registerData() {
      ...
      this.dataManager.register(BOAT_TYPE, SubmarineEntity.Type.BS.ordinal());
...
public boolean attackEntityFrom(DamageSource source, float amount) {
   ...
   public void applyEntityCollision(Entity entityIn) {
      if (entityIn instanceof SubmarineEntity) {
   ...
   public void tick() {
      ...
      if (this.status != SubmarineEntity.Status.UNDER_FLOWING_WATER) {
    ...
    */
   private SubmarineEntity.Status getBoatStatus() {
      SubmarineEntity.Status boatentity$status = this.getUnderwaterStatus();
      if (boatentity$status != null) {
         this.waterLevel = this.getBoundingBox().maxY;
         return boatentity$status;
      } else if (this.checkInWater()) {
         return SubmarineEntity.Status.IN_WATER;
      } else {
         float f = this.getBoatGlide();
         if (f > 0.0F) {
            this.boatGlide = f;
            return SubmarineEntity.Status.ON_LAND;
         } else {
            return SubmarineEntity.Status.IN_AIR;
         }
      }
   }
    ...
    */
   @Nullable
   private SubmarineEntity.Status getUnderwaterStatus() {
      AxisAlignedBB axisalignedbb = this.getBoundingBox();
               ...
               for(int i2 = i1; i2 < j1; ++i2) {
                  ...
                  if (ifluidstate.isTagged(FluidTags.WATER) && d0 < (double)((float)blockpos$pooledmutableblockpos.getY() + ifluidstate.getActualHeight(this.world, blockpos$pooledmutableblockpos))) {
                     if (!ifluidstate.isSource()) {
                        SubmarineEntity.Status boatentity$status = SubmarineEntity.Status.UNDER_FLOWING_WATER;
                        return boatentity$status;
                     }
      ...
      return flag ? SubmarineEntity.Status.UNDER_WATER : null;
   }
   ...
   private void updateMotion() {
      ...
      double d2 = 0.0D;
      this.momentum = 0.05F;
      if (this.previousStatus == SubmarineEntity.Status.IN_AIR && this.status != SubmarineEntity.Status.IN_AIR && this.status != SubmarineEntity.Status.ON_LAND) {
         ...
         this.lastYd = 0.0D;
         this.status = SubmarineEntity.Status.IN_WATER;
      } else {
         if (this.status == SubmarineEntity.Status.IN_WATER) {
            d2 = (this.waterLevel - this.getBoundingBox().minY) / (double)this.getHeight();
            this.momentum = 0.9F;
         } else if (this.status == SubmarineEntity.Status.UNDER_FLOWING_WATER) {
            d1 = -7.0E-4D;
            this.momentum = 0.9F;
         } else if (this.status == SubmarineEntity.Status.UNDER_WATER) {
            d2 = (double)0.01F;
            this.momentum = 0.45F;
         } else if (this.status == SubmarineEntity.Status.IN_AIR) {
            this.momentum = 0.9F;
         } else if (this.status == SubmarineEntity.Status.ON_LAND) {
   ...
   protected void readAdditional(CompoundNBT compound) {
      if (compound.contains("Type", 8)) {
         this.setBoatType(SubmarineEntity.Type.getTypeFromString(compound.getString("Type")));
      }
      ...
      if (!this.isPassenger()) {
         if (onGroundIn) {
            if (this.fallDistance > 3.0F) {
               if (this.status != SubmarineEntity.Status.ON_LAND) {
               ...
               if (!this.world.isRemote && !this.removed) {
                  ...
                  if (this.world.getGameRules().getBoolean(GameRules.DO_ENTITY_DROPS)) {
                     for(int i = 0; i < 3; ++i) {
                        this.entityDropItem(this.getBoatType().asBlock());
                     }
   ...
   public void setBoatType(SubmarineEntity.Type boatType) {
      this.dataManager.set(BOAT_TYPE, boatType.ordinal());
   }

   public SubmarineEntity.Type getBoatType() {
      return SubmarineEntity.Type.byId(this.dataManager.get(BOAT_TYPE));
   }
   ...
   public static enum Type {
      BS(BlockList.blue_steel_block, "bs");

      private final String name;
      private final Block block;

      private Type(Block material, String name) {
         this.name = name;
         this.block = material;
      }
       ...
       */
      public static SubmarineEntity.Type byId(int id) {
         SubmarineEntity.Type[] aboatentity$type = values();
      ...
      public static SubmarineEntity.Type getTypeFromString(String nameIn) {
         SubmarineEntity.Type[] aboatentity$type = values();
